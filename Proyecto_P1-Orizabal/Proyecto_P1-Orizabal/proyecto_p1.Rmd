...
merge(airports, flights, by.x = "faa", by.y = "dest") %>%  group_by(faa,name)  %>% summarise(retraso = mean(arr_delay, na.rm = TRUE)) %>% arrange(retraso)   %>% distinct(faa, retraso,name) %>%head(3)

#Resultado del comando: 
#faa   name                      retraso
#<chr> <chr>                       <dbl>
#1 LEX   Blue Grass                 -22   
#2 PSP   Palm Springs Intl          -12.7 
#3 SNA   John Wayne Arpt Orange Co   -7.87

# Respuesta: Los 3 aeropuertos principales son: 1.-  Blue Grass, 2.-  Palm Springs Intl , 3.- John Wayne Arpt Orange Co
